import glob
from .utils import BOHOUR_NAMES,label2name, char2idx, vocab, BOHOUR_NAMES_AR
...
class BaitAnalysis:
  ...
  def analyze(self, baits=None, diacritized_baits = None, read_from_path='/content/qawafi/demo',return_closest_baits=True,short_qafiyah=False):
    if baits is not None and diacritized_baits is not None:
        ...
        diacritized_baits = open(f'{read_from_path}/baits_output.txt', 'r').read().splitlines()
    else:
        raise Exception("either baits list should be provided or read_from_file should be True")
